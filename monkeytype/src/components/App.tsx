...
export default function App() {
  ...
  function changeChartData(timestamp:Number){
    ...
    setChartData((prev:any) => {
      ...
      return [[...prev[0], x],[...prev[1], Number(((inp.length / x) * 60).toFixed(0))]]
    })
  }
  ...
  function overallValidate(){
    if(promptTxt.split(' ').length < count) return false
    if(validatePrompt(haveNumbers,havePunctuation) === false) return false
    return true
  }
  useEffect(()=>{
    generateWords(count,havePunctuation,haveNumbers)
  ...
  <path d="M11 5.466V4H5a4 4 0 0 0-3.584 5.777.5.5 0 1 1-.896.446A5 5 0 0 1 5 3h6V1.534a.25.25 0 0 1 .41-.192l2.36 1.966c.12.1.12.284 0 .384l-2.36 1.966a.25.25 0 0 1-.41-.192m3.81.086a.5.5 0 0 1 .67.225A5 5 0 0 1 11 13H5v1.466a.25.25 0 0 1-.41.192l-2.36-1.966a.25.25 0 0 1 0-.384l2.36-1.966a.25.25 0 0 1 .41.192V12h6a4 4 0 0 0 3.585-5.777.5.5 0 0 1 .225-.67Z"/>
      ...
      <h1 className='text-xl'>{overallValidate() === false ? 'Generating...' : promptTxt}</h1>
    </div>
    <div className={`p-6 ${textColor}`}>
